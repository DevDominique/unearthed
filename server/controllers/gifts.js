/*Store controller f(x)'s to perform CRUD ops
on gifts table*/
...
const getGifts = async (req, res) => {
    ...
    try{
        //make query to db to req all rows from gifts table
        const results = await pool.query('SELECT * FROM gifts ORDER BY id ASC');
        res.status(200).json(results.rows);
    } catch (error) {
        /* 409 = req, server, rte exists but server cannot perform
        action because of an inconsistency or conflict in req */
